...
public:
    ...
    bool    __accurate_gain_ratio;       //(default true) accurate gain ratio for approximate solutions

    //////////////////////////////
    float   __lm_delta_threshold;        //(default 1e-6)|dx|_2, I use absolute (not relative) change
    float   __lm_gradient_threshold;     //(default 1e-10)|Jt * e|_inf
    float   __lm_mse_threshold;          //(default 0.25) quit if MSE is equal to or smaller than this
    ...
    float   __cg_norm_threshold;         //(default 0.1)terminate CG if norm ratio is less than threshold
    ...
    bool    __fixed_intrinsics;         //(default false) set true for calibrated camera system
    int     __use_radial_distortion;     //(default 0, 1 for projection distortion, 2 for measurement distortion)
    bool    __reset_initial_distortion;  //(default false) reset the initial distortio to 0
    ...
    ///////////////////////////////////
    bool    __jacobian_normalize;        //(default true) scaling the jacobians according to initial jacobians
...
public:
    ...
    double  GetBundleTiming(int timer = TIMER_OVERALL) {return __timer_record[timer];}
